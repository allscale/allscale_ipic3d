...
namespace ipic3d {
	...
	void simulateSteps(const UseCase& useCase, int numSteps, double dt, Universe& universe) {
		...
		for(int i = 0; i<numSteps; ++i) {
			...
			pfor(zero,size,[&](const utils::Coordinate<3>& pos){
				fieldSolver(useCase, universe.cells[pos], pos, universe.field);
			});
	...
	namespace detail {
		...
		struct default_field_solver {
			...
			void operator()(const UseCase& useCase, const Cell& cell, const utils::Coordinate<3>& pos, Field& field) const {
				// the default does not do anything here
				FieldSolver(useCase, cell, pos, field);
			}
